...
- Новый глиф `chapter_plate` (`U+E221`) — лента 200×32 под заголовок раздела
  прогрессии, `ascent 20`, поэтому текст поверх неё оказывается по центру без
  подбора вертикали. Нарисована в оттенках серого: клиент умножает глиф на цвет
  текста, поэтому один PNG принимает любой градиент и новому разделу не нужны
  ни своя картинка, ни новый код-поинт, ни релиз GuiOk. Тело ленты тёмное, а
  рамка светлая — заголовок пишется поверх, и на светлой заливке светлый текст
  не читался бы.
