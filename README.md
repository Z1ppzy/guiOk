...
| Глиф | Символ | Метрики |
| --- | --- | --- |
| логотип | `U+E001` | height 22, ascent 20 |
| монетка | `U+E002` | height 11, ascent 10 |
| ESC-логотип | `U+E100` | height 64, ascent 16 |

Пример для TAB — `&`-коды и плейсхолдеры продолжают работать как обычно:

```yaml
header-footer:
  footer:
    - "&7Баланс: &a%vault_eco_balance%\uE002"
```

`"\uE002"` — YAML-эскейп, он работает только в двойных кавычках; иначе вставьте
сам символ. Тег `<font:guiok:hud>` тоже остаётся рабочим вариантом там, где
плагин парсит MiniMessage, но он не нужен.

Отличие от `<icon:coin>`: собственный sidebar скрывает иконку, пока пак не
загружен, а сторонний плагин про состояние пака не знает и покажет игроку без
пака пустой квадрат. Логотип высотой 22 пикселя в шапке таблиста перекрывает
соседнюю строку — под такие места заведите отдельный provider с меньшими
`height` и `ascent`.
