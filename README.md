...
| `gui_backpack` | `U+E220` | панель заголовка GUI 176×17, `ascent 13` |
| `chapter_plate` | `U+E221` | лента-баннер 200×32, `ascent 20`, красится цветом текста |
...
`chapter_plate` нарисован в оттенках серого намеренно. Клиент умножает глиф
шрифта на цвет текста, поэтому одна и та же лента принимает любой градиент, и
разным разделам прогрессии не нужны разные картинки:

```text
<gradient:#FBBF24:#F97316><icon:chapter_plate></gradient>
```

Тело ленты тёмное, а рамка светлая — заголовок пишется **поверх** плашки, и на
светлой заливке светлый текст не читался бы. `ascent 20` при высоте 32 ставит
середину ленты на середину строки, поэтому текст оказывается по центру плашки
без подбора вертикали. Ширина — 200 пикселей, вместе с пробелом после глифа
курсор уходит на 201.

### Отступы и картинка в GUI

Пак регистрирует space-провайдер: степени двойки от 1 до 256 пикселей в обе
стороны, `U+E300…` вправо и `U+E310…` влево. Любой отступ до ±511 собирается из
них, для этого есть `GuiOkApi#space(int)`.

Отрицательный отступ — единственный способ нарисовать картинку не там, где уже
стоит текст. Заголовок сундука рисуется в точке `(8, 6)` окна, поэтому панель
шириной 176 пикселей ставится так:

```text
space(-8) + glyph("gui_backpack") + space(-169) + "Заголовок"
```

`-8` уводит курсор к левому краю окна, панель занимает 177 пикселей вместе с
пробелом после глифа, `-169` возвращает курсор к обычной позиции заголовка, и
текст ложится поверх панели. `gui_backpack` высотой 17 пикселей при `ascent 13`
закрывает ровно полосу над первым рядом ячеек и не задевает предметы.
...
- `wait-for-pack: false` (умолчание) показывает sidebar сразу с текстовым
  заголовком и переключает его на packed, когда пак применится: игрок не сидит
  без HUD, но видит подмену. `true` убирает подмену ценой пустого экрана до
  конца скачивания. Квадрат отсутствующего глифа не появится ни там, ни там —
  без пака `<icon:…>` схлопывается в пустоту, а заголовок берётся текстовый.
- `delay-ticks: 0` (умолчание) отправляет пак на следующем тике после входа.
  Если промпт проскакивает, не дав нажать «принять», поднимите до `5`.
