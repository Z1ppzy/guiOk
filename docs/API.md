...
не поместившиеся предметы так же, как Bukkit `Inventory#addItem`. Количество
ограничено `GuiOkItemService.MAX_GIVE_AMOUNT` (6400) — инвентарь вмещает
заметно меньше, а неограниченное значение выделило бы миллионы стаков только
для того, чтобы тут же вернуть их обратно.
